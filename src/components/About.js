...
export default function About() {
    const[stylemy, setstylemy]=useState({
        color:'black',
        backgroundColor:'white',
        border:'2 px solid white'
    })
    const[btntext ,setBtntex]=useState("Dark mode Enable")
    const toglestyle=()=>{
        if(stylemy.color === 'white'){
            setstylemy({
                color:'black',
                backgroundColor:'white'
            })
            setBtntex("light mode Enable")
        }
        else{
            setstylemy({
                color:'white',
    ...
    return (
        ...
        <div className="container my-5">
            <button type="button" className="btn btn-success" onClick={toglestyle}>{btntext}</button>
